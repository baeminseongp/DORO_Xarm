...
class TrainingNode(Node):
    ...
    def model_training(self):
        for epoch in range(1, self.epochs + 1):
            self.model.train_dataset(self.trainloader, log_interval=10, epoch=epoch)
    ...
    def load_dataset(self):
        try:
            ...
            with zipfile.ZipFile(self.train_path + ".zip", 'r') as zip_ref:
                zip_ref.extractall('/'.join(self.train_path.split('/')[:-1]))
            trainset = torchvision.datasets.ImageFolder(root=self.train_path,
                                                        transform= self.trans)
        try:
            testset = torchvision.datasets.ImageFolder(root=self.test_path,
                                                    transform= self.trans)
            self.get_logger().info(f"Extracted {self.test_path}")
        except:
            with zipfile.ZipFile(self.test_path + ".zip", 'r') as zip_ref:
                zip_ref.extractall('/'.join(self.train_path.split('/')[:-1]))
            testset = torchvision.datasets.ImageFolder(root=self.test_path,
                                                       transform= self.trans)
    ...
    def run(self):
        ...
        torch.save(self.model.state_dict(), f'{self.model_path}/modelcnn.pt')
